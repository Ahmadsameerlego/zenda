        ...
        <StorefrontLayout store={store} pages={pages} isArabic={isArabic}>
            ...
            <div className="bg-white border-b border-gray-100 py-6">
                <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between">
                    <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-400 font-black hover:text-gray-900 uppercase tracking-widest text-xs">
                        <ChevronLeft className="size-4" />
                        {isArabic ? 'العودة للمتجر' : 'Back to Store'}
                    </button>
                    <div className="text-center">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">{isArabic ? 'إتمام الطلب آمن' : 'SECURE CHECKOUT'}</p>
                    </div>
                </div>
            </div>
                ...
                {step === 'checkout' ? (
                    <div className="flex flex-col lg:flex-row gap-12 lg:gap-24">
                        ...
                        <div className="flex-[1.5] space-y-12">
                            <div className="space-y-10">
                                <div className="flex items-center gap-5">
                                    ...
                                    <h2 className="text-4xl font-[1000] text-gray-900 tracking-tighter">
                                ...
                                <form onSubmit={handleSubmit} id="checkout-form" className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-3 md:col-span-2">
                                        <Label className="text-xs font-black text-gray-400 uppercase tracking-widest mr-1">{isArabic ? 'الاسم بالكامل' : 'Full Name'}</Label>
                                        <div className="relative group">
                                            <User className="absolute right-5 top-1/2 -translate-y-1/2 size-5 text-gray-300 group-focus-within:text-gray-900 transition-colors" />
                                            <Input
                                                required
                                                value={formData.name}
                                                onChange={e => setFormData(f => ({ ...f, name: e.target.value }))}
                                                placeholder={isArabic ? 'أدخل اسمك هنا...' : 'Enter your name...'}
                                                className="h-16 pr-14 rounded-[1.25rem] border-gray-100 bg-white font-bold focus:shadow-2xl focus:border-gray-900 transition-all text-lg"
                                            />
                                        </div>
                                    </div>

                                    <div className="space-y-3">
                                        <Label className="text-xs font-black text-gray-400 uppercase tracking-widest mr-1">{isArabic ? 'رقم الهاتف' : 'Phone Number'}</Label>
                                        <div className="relative group">
                                            <Phone className="absolute right-5 top-1/2 -translate-y-1/2 size-5 text-gray-300 group-focus-within:text-gray-900 transition-colors" />
                                            <Input
                                                required
                                                type="tel"
                                    ...
                                    <div className="space-y-3">
                                        <Label className="text-xs font-black text-gray-400 uppercase tracking-widest mr-1">{isArabic ? 'المدينة' : 'City'}</Label>
                                        <Input
                                            required
                                            value={formData.city}
                                            onChange={e => setFormData(f => ({ ...f, city: e.target.value }))}
                                            placeholder={isArabic ? 'مثال: القاهرة' : 'e.g. Cairo'}
                                            className="h-16 px-6 rounded-[1.25rem] border-gray-100 bg-white font-bold focus:shadow-2xl focus:border-gray-900 transition-all text-lg"
                                        />
                                    </div>

                                    <div className="space-y-3 md:col-span-2">
                                        <Label className="text-xs font-black text-gray-400 uppercase tracking-widest mr-1">{isArabic ? 'العنوان بالتفصيل' : 'Detailed Address'}</Label>
                                        <div className="relative group">
                                            <MapPin className="absolute right-5 top-6 size-5 text-gray-300 group-focus-within:text-gray-900 transition-colors" />
                                            <textarea
                                                required
                                                value={formData.address}
                                                onChange={e => setFormData(f => ({ ...f, address: e.target.value }))}
                                                placeholder={isArabic ? 'اسم الشارع، رقم المبنى، الشقة...' : 'Street name, bldg no, flat...'}
                                                className="w-full min-h-[140px] pr-14 pt-6 rounded-[1.25rem] border border-gray-100 bg-white font-bold focus:shadow-2xl focus:border-gray-900 transition-all outline-none p-6 text-lg"
                                            />
                                        </div>
                                    </div>
                                </form>
                        ...
                        <div className="flex-1">
                            <div className="bg-white rounded-[3rem] border border-gray-100 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)] p-10 md:p-12 sticky top-36 space-y-10">
                                <h3 className="text-3xl font-[1000] text-gray-900 tracking-tighter border-b border-gray-50 pb-6">{isArabic ? 'ملخص الطلب' : 'Order Summary'}</h3>

                                <div className="space-y-8 max-h-[400px] overflow-y-auto pr-2 no-scrollbar">
                                    {cartItems.length > 0 ? cartItems.map((item: any) => (
                                        <div key={item.id} className="flex gap-6 items-center">
                                            <div className="size-20 rounded-[1.5rem] bg-gray-50 border border-gray-100 overflow-hidden shrink-0 shadow-sm">
                                                <img src={item.image} className="w-full h-full object-cover" />
                                            </div>
                                            <div className="flex-1 space-y-1.5">
                                                <h4 className="font-black text-gray-900 leading-tight">{item.name}</h4>
                                                <div className="flex items-center justify-between">
                                                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest">{isArabic ? 'الكمية' : 'Qty'}: {item.quantity}</p>
                                                    <p className="font-black text-base" style={{ color: primaryColor }}>{(item.price || 0).toLocaleString()} ج.م</p>
                                                </div>
                                            </div>
                                        </div>
                                    )) : (
                                        <div className="py-12 text-center text-gray-400 font-black">{isArabic ? 'سلتك فارغة' : 'YOUR CART IS EMPTY'}</div>
                                    )}
                                </div>

                                <div className="space-y-5 pt-8 border-t border-gray-50">
                                    <div className="flex justify-between text-gray-400 font-black text-sm uppercase tracking-widest">
                                        <span>{isArabic ? 'المجموع الفرعي' : 'Subtotal'}</span>
                                        <span className="text-gray-900">{(totals.subtotal || 0).toLocaleString()} ج.م</span>
                                    </div>
                                    <div className="flex justify-between text-gray-400 font-black text-sm uppercase tracking-widest">
                                        <span>{isArabic ? 'تكلفة الشحن' : 'Shipping'}</span>
                                        <span className="text-gray-900">{(totals.shipping || 0).toLocaleString()} ج.م</span>
                                    </div>
                                    <div className="flex justify-between pt-6 border-t border-gray-100">
                                        <span className="text-2xl font-black text-gray-900 tracking-tight">{isArabic ? 'الإجمالي' : 'Total'}</span>
                                ...
                                <div className="flex items-center justify-center gap-4 py-2 opacity-50">
                                    ...
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{isArabic ? 'تشفير آمن للبيانات 100%' : '100% SECURE & ENCRYPTED'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="max-w-2xl mx-auto py-20 text-center space-y-12">
                        ...
                        <div className="space-y-6">
                            <h1 className="text-5xl md:text-7xl font-[1000] text-gray-900 tracking-tighter">
                        ...
                        <div className="p-10 rounded-[3rem] bg-white border border-gray-100 shadow-2xl space-y-8">
                            <Button
