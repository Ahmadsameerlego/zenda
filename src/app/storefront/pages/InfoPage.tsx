...
export default function InfoPage() {
    ...
    return (
        <StorefrontLayout store={store} pages={allPages} isArabic={isArabic}>
            {/* Header / Banner */}
            <section className="bg-gray-50 border-b border-gray-100 py-24 md:py-32">
                <div className="max-w-4xl mx-auto px-6 space-y-6">
                    <button
                        onClick={() => navigate(`/store/${slug}`)}
                        className="flex items-center gap-2 text-gray-400 font-black hover:text-gray-900 transition-colors uppercase tracking-widest text-xs"
                    >
                        <ChevronLeft className="size-4" />
                        {isArabic ? 'العودة للرئيسية' : 'Back Home'}
                    </button>
                    <h1 className="text-4xl md:text-7xl font-[1000] text-gray-900 tracking-tighter">
                        {page.title}
                    </h1>
            ...
            <main className="max-w-4xl mx-auto px-6 py-20 md:py-32">
                <article className="prose prose-2xl prose-gray max-w-none">
                    <div
                        className="text-xl md:text-2xl text-gray-600 leading-[1.8] font-medium whitespace-pre-line"
                ...
                {allPages.length > 1 && (
                    <div className="mt-32 pt-20 border-t border-gray-100 space-y-8">
                        ...
                        <div className="flex flex-wrap gap-4">
                            {allPages.filter(p => p.page_key !== pageKey).map(p => (
                                <button
                                    ...
                                    className="px-8 h-14 rounded-2xl bg-gray-50 text-gray-900 font-black hover:bg-gray-900 hover:text-white transition-all border border-gray-100"
                                >
                                    {p.title}
                                </button>
                            ))}
